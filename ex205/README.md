...
* If you are using k3d, import the 2 docker images into the registry by running `k3d image import dilansachi/devopswk8s-ex205-log-reader:1.0.0` and `k3d image import dilansachi/devopswk8s-ex205-ping-pong-application:1.0.0`.
* Execute `kubectl apply -f manifests/` in the `ex205` directory to deploy the pod with the 2 containers.
