...
#### Build & run locally
* Execute `./gradle clean build` inside the `ex113/image_application` directory. This will build the log reader java project, create a jar and then will create a docker image with the jar file encapsulated.
* If you are using k3d, import the docker image into the registry by running `k3d image import dilansachi/devopswk8s-ex113-image-application:1.0.0`.
* Execute `kubectl apply -f manifests/` in the `ex113` directory to deploy the pod.
