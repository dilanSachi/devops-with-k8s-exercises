...
BROADCASTER_APP_IMAGE_TAG="$4"

if [[ -n "$TODO_APP_IMAGE_TAG" ]]
then
  echo "Build image tag received as input: $TODO_APP_IMAGE_TAG"
else
    TODO_APP_IMAGE_TAG="dilansachi/devopswk8s-ex409-todo-app:1.0.0"
...
then
  echo "Build image tag received as input: $TODO_BACKEND_IMAGE_TAG"
    ...
    TODO_BACKEND_IMAGE_TAG="dilansachi/devopswk8s-ex409-todo-backend:1.0.0"
    echo "Using default image tag: $TODO_BACKEND_IMAGE_TAG"
fi

if [[ -n "$BROADCASTER_APP_IMAGE_TAG" ]]
then
  echo "Build image tag received as input: $BROADCASTER_APP_IMAGE_TAG"
else
    BROADCASTER_APP_IMAGE_TAG="dilansachi/devopswk8s-ex409-broadcaster-app:1.0.0"
    echo "Using default image tag: $BROADCASTER_APP_IMAGE_TAG"
fi
...
cd ${DIR}/broadcaster_app && ./gradlew clean build -Pimage_tag="$BROADCASTER_APP_IMAGE_TAG" -Pskip_local_image_build=$SKIP_LOCAL_IMAGE_BUILD
